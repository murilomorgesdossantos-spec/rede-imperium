...
// ============================================================
// CARREGAR LISTA DE MINAS PRIVADAS
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No minas-privadas.html você precisa colocar id="lista-minas" na div grid
    const container = document.getElementById('lista-minas');
    if (!container) return;

    fetch('./constants/minas-privadas.json')
        .then(res => res.json())
        .then(minas => {
            listaMinasData = minas;
            let htmlContent = '';
        ...
        .catch(err => console.error("Erro ao carregar Minas Privadas:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaMinasData.length === 0) return;
    const item = listaMinasData.find(i => i.slug === slug);
