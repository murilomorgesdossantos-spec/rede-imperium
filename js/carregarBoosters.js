...
// ============================================================
// CARREGAR LISTA DE BOOSTERS
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No boosters.html você precisa colocar id="lista-boosters" na div grid
    const container = document.getElementById('lista-boosters');
    if (!container) return;

    fetch('./constants/boosters.json')
        .then(res => res.json())
        .then(boosters => {
            listaBoostersData = boosters;
            let htmlContent = '';
        ...
        .catch(err => console.error("Erro ao carregar Boosters:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaBoostersData.length === 0) return;
    const item = listaBoostersData.find(i => i.slug === slug);
