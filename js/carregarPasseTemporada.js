...
// ============================================================
// CARREGAR LISTA DO PASSE
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No passe-de-temporada.html você precisa colocar id="lista-passe" na div grid
    const container = document.getElementById('lista-passe');
    if (!container) return;

    fetch('./constants/passe-de-temporada.json')
        .then(res => res.json())
        .then(passes => {
            listaPasseData = passes;
            let htmlContent = '';
        ...
        .catch(err => console.error("Erro ao carregar Passe de Temporada:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaPasseData.length === 0) return;
    const item = listaPasseData.find(i => i.slug === slug);
