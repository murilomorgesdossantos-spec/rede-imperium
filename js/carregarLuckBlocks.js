...
// ============================================================
// CARREGAR LISTA DE LUCK BLOCKS
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No luck-blocks.html você precisa colocar id="lista-luck-blocks" na div grid
    const container = document.getElementById('lista-luck-blocks');
    if (!container) return;

    fetch('./constants/luck-blocks.json')
        .then(res => res.json())
        .then(items => {
            listaLuckBlocksData = items;
            let htmlContent = '';
        ...
        .catch(err => console.error("Erro ao carregar Luck Blocks:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaLuckBlocksData.length === 0) return;
    const item = listaLuckBlocksData.find(i => i.slug === slug);
