...
// CONFIGURAÇÃO
// ============================================================
// ATENÇÃO: Verifique se a URL é a mesma que você usou nos outros arquivos
...
// ============================================================
// CARREGAR LISTA DE REVOGAÇÕES
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No revogacoes.html você precisa colocar id="lista-revogacoes" na div grid
    const container = document.getElementById('lista-revogacoes');
    if (!container) return;

    fetch('./constants/revogacoes.json')
        .then(res => res.json())
        .then(revogacoes => {
            listaRevogacoesData = revogacoes;
            let htmlContent = '';

            revogacoes.forEach(item => {
                const badge = item.isNew ? `<span class="new">NOVIDADE</span>` : '';
                const nomeSeguro = String(item.name).replace(/"/g, '&quot;');
                
                // Lógica visual para desconto (se for 0, esconde a porcentagem)
        ...
        .catch(err => console.error("Erro ao carregar Revogações:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaRevogacoesData.length === 0) return;
    const item = listaRevogacoesData.find(i => i.slug === slug);
