...
// ============================================================
// CARREGAR LISTA DE KITS ESPECIAIS
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No kits-especiais.html você precisa colocar id="lista-kits-especiais" na div grid
    const container = document.getElementById('lista-kits-especiais');
    if (!container) return;

    fetch('./constants/kits-especiais.json')
        .then(res => res.json())
        .then(kits => {
            listaKitsData = kits;
            let htmlContent = '';
        ...
        .catch(err => console.error("Erro ao carregar Kits Especiais:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaKitsData.length === 0) return;
    const item = listaKitsData.find(i => i.slug === slug);
