...
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No tags.html você precisa colocar id="lista-tags" na div grid
    const container = document.getElementById('lista-tags');
    if (!container) return;

    fetch('./constants/tags.json')
        .then(res => res.json())
        .then(tags => {
            listaTagsData = tags;
            let htmlContent = '';
...
window.abrirModalDetalhes = function(slug) {
    if (listaTagsData.length === 0) return;
    const item = listaTagsData.find(i => i.slug === slug);
