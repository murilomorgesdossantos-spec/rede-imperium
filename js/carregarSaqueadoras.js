...
// ============================================================
// CARREGAR LISTA DE SAQUEADORAS
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // IMPORTANTE: No saqueadoras.html você precisa colocar id="lista-saqueadoras" na div grid
    const container = document.getElementById('lista-saqueadoras');
    if (!container) return;

    fetch('./constants/saqueadoras.json')
        .then(res => res.json())
        .then(saqueadoras => {
            listaSaqueadorasData = saqueadoras;
            let htmlContent = '';

            saqueadoras.forEach(item => {
        ...
        .catch(err => console.error("Erro ao carregar Saqueadoras:", err));
});
...
window.abrirModalDetalhes = function(slug) {
    if (listaSaqueadorasData.length === 0) return;
    const item = listaSaqueadorasData.find(i => i.slug === slug);
